...
The X2 USD keeps URDF fixed frames (e.g. ``rgbd_head_front``) as stage **Xform**
prims under ``head_pitch_link``, but they are **not** PhysX articulation bodies
(``robot.data.body_names`` only lists the 32 DOF links). CameraCfg must parent to
the existing Xform path:

    {ENV_REGEX_NS}/Robot/head_pitch_link/rgbd_head_front/depth_cam

Extrinsics are already in that Xform (from URDF); use identity offset.

Current phase: scene mount + realtime depth access / visualization only.
Do **not** wire into policy/critic observations here.
"""
