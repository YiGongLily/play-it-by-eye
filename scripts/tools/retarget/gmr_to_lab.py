...
Output Legged Lab Format:
    ...
    - 'root_rot': Root rotation quaternions, shape (num_frames, 4), format (w, x, y, z)
      Note: Isaac Lab v3 sim APIs use XYZW; this script writes XYZW to sim for FK, then
      converts back to WXYZ for the pkl (MotionDataManager expects WXYZ and converts on load).
    - 'dof_pos': Degrees of freedom positions, shape (num_frames, num_dofs)
...
def run_simulator(
    ...
    for motion_data in motion_data_dicts:
        ...
        # GMR stores root_rot as XYZW. Isaac Lab v3 write_root_state also expects XYZW.
        # (Do NOT convert to WXYZ before writing to sim — that flips the robot ~180° about X.)
        root_quat_xyzw = torch.from_numpy(motion_data["root_rot"]).to(scene.device).float()
        root_quat_xyzw = math_utils.normalize(root_quat_xyzw)
        # quat_unique assumes WXYZ (positive w); apply on WXYZ then convert back.
        root_quat_wxyz = math_utils.convert_quat(root_quat_xyzw, "wxyz")
        root_quat_wxyz = math_utils.quat_unique(root_quat_wxyz)
        root_quat_xyzw = math_utils.convert_quat(root_quat_wxyz, "xyzw")
        root_quat_list.append(root_quat_xyzw)
    ...
    # Persist LeggedLab pkl convention: root_rot as WXYZ (MotionDataManager converts to XYZW on load).
    for motion_data_dict, root_quat_xyzw in zip(motion_data_dicts, root_quat_list):
        root_quat_wxyz = math_utils.convert_quat(root_quat_xyzw, "wxyz")
        motion_data_dict["root_rot"] = root_quat_wxyz.cpu().numpy()
